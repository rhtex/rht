...
<div class="row mb-2">
    <div class="col-sm-6">
        <h1>Twisting DC Details</h1>
    </div>
    <div class="col-sm-6 text-end">
        <a href="<?= site_url('production/yarn-twisting') ?>" class="btn btn-secondary"><i class="fas fa-list"></i>
            List</a>
        <?php if ($dc['status'] !== 'Completed' && $dc['status'] !== 'Cancelled'): ?>
            <a href="<?= site_url('production/yarn-twisting/receipt-create/' . $dc['id']) ?>" class="btn btn-success"><i
                    class="fas fa-download"></i> Receive Yarn</a>
        <?php endif; ?>
    </div>
</div>
<?= $this->endSection() ?>
                ...
                <div class="card card-outline card-success mb-3">
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                <strong>Receipt No: <?= esc($r['receipt_number']) ?></strong> | Date:
                                <?= esc($r['receipt_date']) ?>
                            </div>
                            <div class="col-md-6 text-end">
                                <a href="<?= site_url('production/yarn-twisting/receipt-delete/' . $r['id']) ?>"
                                    class="btn btn-xs btn-danger" onclick="return confirm('Revert and delete this receipt?')"><i
                                        class="fas fa-trash"></i> Delete Receipt</a>
                            </div>
                        </div>
                    </div>
