...
<body onload="window.print()">
    <div class="box">
        <div class="header">
            ...
            <div class="details">
                <h2>SALES ORDER</h2>
                <p><strong># :</strong> <?= esc($order['sales_order_number']) ?></p>
                <p><strong>Date :</strong> <?= date('d/m/Y', strtotime($order['order_date'])) ?></p>
                <?php if ($order['shipment_date']): ?><p><strong>Expected Shipment :</strong> <?= date('d/m/Y', strtotime($order['shipment_date'])) ?></p><?php endif; ?>
                <?php if ($order['transport_name']): ?><p><strong>Transport :</strong> <?= esc($order['transport_name']) ?></p><?php endif; ?>
                <?php if ($order['waybill_number']): ?><p><strong>Waybill :</strong> <?= esc($order['waybill_number']) ?></p><?php endif; ?>
            </div>
        </div>
        ...
        <div class="totals">
            <table>
                ...
                <?php if ($order['discount_amount'] > 0): ?>
                    ...
                    <tr><td>Discount (<?= $order['discount_type'] ?>):</td><td class="text-right">-₹<?= number_format($actualDisc, 2) ?></td></tr>
                <?php endif; ?>
                <tr><td>Tax:</td><td class="text-right">₹<?= number_format($order['tax_amount'], 2) ?></td></tr>
                <?php if ($order['shipping_charge'] > 0): ?>
                    <tr><td>Shipping:</td><td class="text-right">₹<?= number_format($order['shipping_charge'], 2) ?></td></tr>
                <?php endif; ?>
                <tr style="border-top: 2px solid #333; font-size: 16px; font-weight: bold;">
                    <td>Total:</td><td class="text-right">₹<?= number_format($order['total_amount'], 2) ?></td>
                </tr>
            </table>
        </div>
        ...
        <div style="clear: both; margin-top: 30px;">
            <?php if ($order['notes']): ?>
                <p><strong>Notes:</strong><br><?= nl2br(esc($order['notes'])) ?></p>
            <?php endif; ?>
            <?php if ($order['terms']): ?>
                <p><strong>Terms:</strong><br><?= nl2br(esc($order['terms'])) ?></p>
            <?php endif; ?>
        </div>
