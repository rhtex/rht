...
<?= $this->section('title') ?>Dyeing DC Details<?= $this->endSection() ?>
<?= $this->section('header') ?>
<div class="row mb-2">
    <div class="col-sm-6">
        <h1 class="fw-bold text-dark"><i class="fas fa-file-invoice-dollar text-primary me-2"></i>Dyeing DC Details</h1>
    </div>
    <div class="col-sm-6 text-end">
        <a href="<?= site_url('production/yarn-dyeing') ?>" class="btn btn-sm btn-outline-secondary rounded-pill px-3 shadow-none"><i class="fas fa-list me-1"></i> List</a>
        <?php if($dc['status'] !== 'Completed' && $dc['status'] !== 'Cancelled'): ?>
            <a href="<?= site_url('production/yarn-dyeing/receipt-create/'.$dc['id']) ?>" class="btn btn-sm btn-success rounded-pill px-3 shadow-sm"><i class="fas fa-download me-1"></i> Receive Yarn</a>
        <?php endif; ?>
    </div>
</div>
<?= $this->endSection() ?>
...
<div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4" style="border-radius: 12px;">
    <div class="card-header border-0 py-3" style="background: linear-gradient(135deg, #0d6efd 0%, #0a58ca 100%);">
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                <div class="p-3 bg-light rounded-3 border border-light-subtle h-100">
                    <span class="text-uppercase text-xs text-muted fw-bold d-block mb-1"><i class="fas fa-user-tie me-1"></i> Vendor (Dyer)</span>
            ...
            <table class="table align-middle table-hover mb-0">
                <thead>
                    <tr class="bg-light">
                        ...
                        <th class="py-2 px-2 text-secondary fw-bold">Required Color</th>
                        <th class="py-2 px-2 text-secondary fw-bold text-end">Qty Issued</th>
                        <th class="py-2 px-2 text-secondary fw-bold text-end">Cones Issued</th>
                        <th class="py-2 px-3 text-secondary fw-bold text-end">Pending Qty</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="py-2 px-2"><strong class="text-primary"><?= esc($item['required_color']) ?></strong></td>
                        <td class="py-2 px-2 text-end"><?= number_format($item['quantity_issued_kg'], 2) ?> Kg</td>
                        <td class="py-2 px-2 text-end fw-semibold"><?= (int)$item['cones_issued'] ?></td>
                    ...
                    <div class="card-header bg-success-subtle border-bottom border-success-subtle py-2">
                        <div class="row align-items-center">
                            ...
                            <div class="col-md-6 text-end">
                                <a href="<?= site_url('production/yarn-dyeing/receipt-view/'.$r['id']) ?>" class="btn btn-xs btn-info text-white rounded-pill px-3 me-1"><i class="fas fa-eye me-1"></i> View</a>
                                <a href="<?= site_url('production/yarn-dyeing/receipt-edit/'.$r['id']) ?>" class="btn btn-xs btn-warning text-dark rounded-pill px-3 me-1"><i class="fas fa-edit me-1"></i> Edit</a>
                                <a href="<?= site_url('production/yarn-dyeing/receipt-delete/'.$r['id']) ?>" class="btn btn-xs btn-danger text-white rounded-pill px-3" onclick="return confirm('Revert and delete this receipt?')"><i class="fas fa-trash me-1"></i> Delete</a>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table align-middle table-sm mb-0">
                            <thead>
                                <tr class="bg-light" style="font-size: 0.72rem;">
                                    ...
                                    <th class="py-2 px-2 text-secondary fw-bold">Received Color</th>
                                    <th class="py-2 px-2 text-secondary fw-bold text-end">Recd Qty</th>
                                    <th class="py-2 px-2 text-secondary fw-bold text-end">Recd Cones</th>
                                    <th class="py-2 px-2 text-secondary fw-bold text-end">Wasted Qty (Kg)</th>
                                    <th class="py-2 px-2 text-secondary fw-bold text-end">Job Charges</th>
                                    <th class="py-2 px-2 text-secondary fw-bold text-end">Landed Cost / Kg</th>
                                    <th class="py-2 px-3 text-secondary fw-bold text-end">Total Landed Value</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="py-2 px-2"><strong class="text-success"><?= esc($ri['received_color']) ?></strong></td>
                                    <td class="py-2 px-2 text-end fw-semibold"><?= number_format($ri['quantity_received_kg'], 2) ?> Kg</td>
                                    <td class="py-2 px-2 text-end fw-semibold"><?= (int)$ri['cones_received'] ?></td>
                                    <td class="py-2 px-2 text-end text-muted"><?= number_format($ri['quantity_wastage_kg'], 2) ?> Kg</td>
                                    <td class="py-2 px-2 text-end">₹<?= number_format($ri['job_work_charges'], 2) ?></td>
                                    <td class="py-2 px-2 text-end fw-bold text-success">₹<?= number_format($ri['landed_cost_per_kg'] ?? 0.00, 2) ?></td>
                                    <td class="py-2 px-3 text-end fw-bold text-success">₹<?= number_format(($ri['quantity_received_kg'] * ($ri['landed_cost_per_kg'] ?? 0.00)), 2) ?></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
