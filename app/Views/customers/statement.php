...
<?= $this->section('title') ?>Statement of Account - <?= esc($customer['name']) ?><?= $this->endSection() ?>

<?= $this->section('header') ?>
<?php if (!isset($is_pdf) || !$is_pdf): ?>
<div class="row align-items-center mb-4 no-print">
    <div class="col-sm-6">
        <h1 class="m-0 text-dark fw-bold">Statement of Account</h1>
        <p class="text-muted mb-0"><?= esc($customer['name']) ?></p>
    </div>
    <div class="col-sm-6 text-end">
        <div class="btn-group">
            ...
            <a href="<?= site_url('customers/view/'.$customer['id']) ?>" class="btn btn-secondary shadow-sm">
        ...
        <table style="width: 100%; border-collapse: collapse; border: none;">
            <tr>
                <td style="width: 50%; vertical-align: top; border: none;">
                    <h2 class="fw-bold text-primary mb-1" style="margin: 0; padding: 0;">STATEMENT</h2>
                    <p class="text-muted" style="margin: 0; padding: 0;">Statement of Account</p>
                    
                    <div style="margin-top: 20px;">
                        <h6 class="text-uppercase text-muted small fw-bold mb-2">Statement For:</h6>
                        <h4 class="fw-bold mb-1" style="margin: 0; padding: 0;"><?= esc($customer['name']) ?></h4>
                        ...
                        <p class="text-muted mt-2">
                            <i class="fas fa-phone-alt me-1 small"></i> <?= esc($customer['phone']) ?>
                        </p>
                    </div>
                </td>
                <td style="width: 50%; vertical-align: top; text-align: right; border: none;">
                    ...
                    <div style="margin-top: 20px;">
                        ...
                        <div>
                            <span class="text-muted small text-uppercase">Account Balance:</span><br>
                            <?php
                            ...
                            <h3 style="margin: 0; padding: 0;" class="fw-bold <?= $finalBalance >= 0 ? 'text-danger' : 'text-success' ?>">
                                ...
                                <small style="font-size: 14px; font-weight: normal;"><?= $finalBalance >= 0 ? 'Dr' : 'Cr' ?></small>
                            </h3>
                        </div>
                    </div>
                </td>
    ...
    <div class="table-responsive">
        <table class="table table-bordered align-middle">
            ...
            <tbody>
                ...
                <?php 
                $runningBalance = ($opening_type == 'Dr' ? $opening_balance : -$opening_balance);
                foreach($transactions as $t): 
            ...
            <tfoot class="bg-light fw-bold">
                <tr>
                    ...
                    <td class="text-end py-3 text-danger">₹<?= number_format($totalDebit, 2) ?></td>
                    <td class="text-end py-3 text-success">₹<?= number_format($totalCredit, 2) ?></td>
                    <td class="text-end py-3">
                        ₹<?= number_format(abs($finalBalance), 2) ?> <?= $finalBalance >= 0 ? 'Dr' : 'Cr' ?>
                    </td>
                </tr>
            </tfoot>
        </table>
