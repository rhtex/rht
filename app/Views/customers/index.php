            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="text" name="search" class="form-control" placeholder="Name, Phone or City..." value="<?= esc($filters['search'] ?? '') ?>">
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="mb-3">
                ...
                <div class="col-md-2">
                    <div class="mb-3">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="<?= site_url('customers') ?>" class="btn btn-outline-secondary" title="Clear Filters"><i class="fas fa-times"></i></a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <table class="table table-bordered table-striped">
                <thead>
                    ...
                    function getSortIcon($field, $currentSortBy, $currentSortOrder) {
                        if ($currentSortBy != $field) return '<i class="fas fa-sort text-muted ms-1"></i>';
                        return $currentSortOrder == 'ASC' ? '<i class="fas fa-sort-up ms-1"></i>' : '<i class="fas fa-sort-down ms-1"></i>';
                    }
                    ?>
                    <tr>
                        <th><a href="<?= getSortLink('customers.id', $currentSortBy, $currentSortOrder) ?>" class="text-dark text-decoration-none">ID <?= getSortIcon('customers.id', $currentSortBy, $currentSortOrder) ?></a></th>
                        <th><a href="<?= getSortLink('customers.name', $currentSortBy, $currentSortOrder) ?>" class="text-dark text-decoration-none">Customer Name (City) <?= getSortIcon('customers.name', $currentSortBy, $currentSortOrder) ?></a></th>
                        <th>Contact</th>
                        <th>GSTIN</th>
                        <th class="text-end"><a href="<?= getSortLink('pending_balance', $currentSortBy, $currentSortOrder) ?>" class="text-dark text-decoration-none">Pending Balance <?= getSortIcon('pending_balance', $currentSortBy, $currentSortOrder) ?></a></th>
                ...
                <tbody id="customerTableBody">
                    <?php if(!empty($customers)): ?>
                        ...
                        <tr>
                            <td><?= $customer['id'] ?></td>
                            <td>
                                <strong><a href="<?= site_url('customers/view/'.$customer['id']) ?>" class="text-dark"><?= esc($customer['name']) ?></a></strong>
                            ...
                            <td class="text-end fw-bold <?= $customer['pending_balance'] >= 0 ? 'text-danger' : 'text-success' ?>">
                                ₹<?= number_format(abs($customer['pending_balance']), 2) ?> <?= $customer['pending_balance'] >= 0 ? 'Dr' : 'Cr' ?>
                            </td>
