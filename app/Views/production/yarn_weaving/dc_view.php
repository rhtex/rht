...
<?= $this->section('title') ?>Weaving DC Details<?= $this->endSection() ?>
<?= $this->section('header') ?>
<div class="row mb-2">
    <div class="col-sm-6">
        <h1 class="fw-bold text-dark"><i class="fas fa-file-invoice text-primary me-2"></i>Weaving DC Details</h1>
    </div>
    <div class="col-sm-6 text-end">
        <a href="<?= site_url('production/yarn-weaving') ?>" class="btn btn-sm btn-outline-secondary rounded-pill px-3 shadow-none"><i class="fas fa-list me-1"></i> List</a>
        <?php if($dc['status'] !== 'Completed' && $dc['status'] !== 'Cancelled'): ?>
            <a href="<?= site_url('production/yarn-weaving/receipt-create/'.$dc['id']) ?>" class="btn btn-sm btn-success rounded-pill px-3 shadow-sm"><i class="fas fa-download me-1"></i> Receive Yarn</a>
        <?php endif; ?>
    </div>
</div>
<?= $this->endSection() ?>
...
<div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4" style="border-radius: 12px;">
    <div class="card-header border-0 py-3" style="background: linear-gradient(135deg, #dc3545 0%, #a71d2a 100%);">
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                <div class="p-3 bg-light rounded-3 border border-light-subtle h-100">
                    <span class="text-uppercase text-xs text-muted fw-bold d-block mb-1"><i class="fas fa-user-tie me-1"></i> Vendor (Weaver)</span>
                    ...
                    <div class="card-header bg-success-subtle border-bottom border-success-subtle py-2">
                        <div class="row align-items-center">
                            ...
                            <div class="col-md-6 text-end">
                                <a href="<?= site_url('production/yarn-weaving/receipt-delete/'.$r['id']) ?>" class="btn btn-xs btn-outline-danger btn-danger-bg rounded-pill px-3" onclick="return confirm('Revert and delete this receipt?')"><i class="fas fa-trash me-1"></i> Delete Receipt</a>
                            </div>
                        </div>
                    </div>
