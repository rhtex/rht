...
<?= $this->section('title') ?>Receive Twisted Yarn Stock<?= $this->endSection() ?>
<?= $this->section('header') ?>
<div class="row mb-2">
    <div class="col-sm-6"><h1>Receive Twisted Yarn (Against DC: <?= esc($dc['dc_number']) ?>)</h1></div>
    <div class="col-sm-6 text-end"><a href="<?= site_url('production/yarn-twisting/view/'.$dc['id']) ?>" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Back</a></div>
</div>
<?= $this->endSection() ?>
<?= $this->section('content') ?>
<div class="card card-outline card-success">
    <form action="<?= site_url('production/yarn-twisting/receipt-store/'.$dc['id']) ?>" method="post" id="receiptForm">
        <?= csrf_field() ?>
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    <label class="form-label">Vendor (Twister)</label>
