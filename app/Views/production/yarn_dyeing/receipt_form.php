...
<?= $this->section('title') ?>Receive Dyed Yarn Stock<?= $this->endSection() ?>
<?= $this->section('header') ?>
<div class="row mb-2">
    <div class="col-sm-6"><h1>Receive Dyed Yarn (Against DC: <?= esc($dc['dc_number']) ?>)</h1></div>
...
<div class="card card-outline card-success">
    <form action="<?= site_url('production/yarn-dyeing/receipt-store/'.$dc['id']) ?>" method="post" id="receiptForm">
        <?= csrf_field() ?>
        <div class="card-body">
            <div class="row">
                ...
                <table class="table table-bordered align-middle">
                    ...
                    <tbody>
                        ...
                        <tr class="item-row" data-item-id="<?= $item['id'] ?>" data-pending="<?= $pending ?>" data-raw-cost="<?= $rawCost ?>">
                            ...
                            <td>
                                <strong><?= esc($item['required_color'] ?: 'Raw') ?></strong>
                                <input type="hidden" name="items[<?= $item['id'] ?>][received_color]" value="<?= esc($item['required_color'] ?: 'Raw') ?>">
                            </td>
                            <td>
                                <input type="text" name="items[<?= $item['id'] ?>][dyeing_lot_number]" class="form-control form-control-sm" placeholder="Lot No.">
                            </td>
