...
use App\Models\YarnTwistingDcModel;
use App\Models\YarnTwistingDcItemModel;
use App\Models\YarnTwistingReceiptModel;
use App\Models\YarnTwistingReceiptItemModel;
use App\Models\YarnStockMovementModel;

class YarnTwistingController extends BaseController
{
    protected $dcModel;
    ...
    {
        $this->dcModel = new YarnTwistingDcModel();
        $this->dcItemModel = new YarnTwistingDcItemModel();
        $this->receiptModel = new YarnTwistingReceiptModel();
        $this->receiptItemModel = new YarnTwistingReceiptItemModel();
    ...
    public function index()
    {
        ...
        $data['title'] = 'Yarn Twisting Job Work';
        return view('production/yarn_twisting/index', $data);
    }
    ...
    public function create()
    {
        ...
        $nextNum = $lastDc ? ((int)str_replace('DC-TWS-', '', $lastDc['dc_number'])) + 1 : 1001;
        $data['nextDcNumber'] = 'DC-TWS-' . $nextNum;

        $data['title'] = 'Create Twisting Delivery Challan';
        return view('production/yarn_twisting/dc_form', $data);
    }
    ...
    public function store()
    {
        ...
        foreach ($items as $item) {
            ...
            $this->movementModel->save([
                ...
                'reference_id'  => $dcId,
                'remarks'       => 'Issued for Twisting to ' . $dcData['vendor_name'] . ' (DC: ' . $dcData['dc_number'] . ')',
                'created_by'    => session('user_id'),
        ...
        return redirect()->to('production/yarn-twisting')->with('success', 'Delivery Challan created successfully.');
    }
    ...
    public function view($id)
    {
        ...
        if (!$data['dc']) {
            return redirect()->to('production/yarn-twisting')->with('error', 'Delivery Challan not found.');
        }

        $data['items'] = $this->dcItemModel->where('dc_id', $id)->findAll();
        $data['title'] = 'Twisting Delivery Challan: ' . $data['dc']['dc_number'];

        $data['receipts'] = $this->receiptModel->where('dc_id', $id)->findAll();
        foreach ($data['receipts'] as &$r) {
            $r['items'] = $this->receiptItemModel
                ->select('production_yarn_twisting_receipt_items.*, production_yarn_twisting_dc_items.mill_name, production_yarn_twisting_dc_items.yarn_count, production_yarn_twisting_dc_items.warp_weft, production_yarn_twisting_dc_items.lot_number, production_yarn_twisting_dc_items.csp, production_yarn_twisting_dc_items.yarn_type, production_yarn_twisting_dc_items.current_color')
                ->join('production_yarn_twisting_dc_items', 'production_yarn_twisting_dc_items.id = production_yarn_twisting_receipt_items.dc_item_id')
                ->where('receipt_id', $r['id'])
                ->findAll();
        }

        return view('production/yarn_twisting/dc_view', $data);
    }
    ...
    public function edit($id)
    {
        ...
        if ($receiptsCount > 0) {
            return redirect()->to('production/yarn-twisting')->with('error', 'Cannot edit because receipts exist.');
        }

        $data['dcItems'] = $this->dcItemModel->where('dc_id', $id)->findAll();
        $data['availableYarns'] = $this->movementModel->getInventory();
        $data['title'] = 'Edit Twisting Delivery Challan';
    ...
    public function update($id)
    {
        ...
        if (!$dc) {
            return redirect()->to('production/yarn-twisting')->with('error', 'DC not found.');
        }

        $receiptsCount = $this->receiptModel->where('dc_id', $id)->countAllResults();
        if ($receiptsCount > 0) {
            return redirect()->to('production/yarn-twisting')->with('error', 'Cannot update because receipts exist.');
        }
        ...
        return redirect()->to('production/yarn-twisting')->with('success', 'Delivery Challan updated successfully.');
    }
    ...
    public function delete($id)
    {
        ...
        $receiptsCount = $this->receiptModel->where('dc_id', $id)->countAllResults();
        if ($receiptsCount > 0) {
            return redirect()->to('production/yarn-twisting')->with('error', 'Cannot delete because receipts exist.');
        }
        ...
        if ($db->transStatus() === false) {
            return redirect()->to('production/yarn-twisting')->with('error', 'Failed to delete Delivery Challan.');
        }

        return redirect()->to('production/yarn-twisting')->with('success', 'Delivery Challan deleted successfully.');
    }
    ...
    public function receiptCreate($dcId)
    {
        ...
        $nextNum = $lastRec ? ((int)str_replace('REC-TWS-', '', $lastRec['receipt_number'])) + 1 : 1001;
        $data['nextReceiptNumber'] = 'REC-TWS-' . $nextNum;

        $data['title'] = 'Receive Twisted Yarn Stock';
        return view('production/yarn_twisting/receipt_form', $data);
    }
    ...
    public function receiptStore($dcId)
    {
        ...
        foreach ($items as $dcItemId => $item) {
            ...
            if ($qtyReceived > 0) {
                $this->movementModel->save([
                    ...
                    'remarks'       => 'Received Twisted from ' . $dc['vendor_name'] . ' against ' . $dc['dc_number'],
                    'created_by'    => session('user_id'),
                    'created_at'    => date('Y-m-d H:i:s')
                ]);
            }
        ...
        return redirect()->to('production/yarn-twisting/view/' . $dcId)->with('success', 'Yarn received successfully.');
    }
    ...
    public function receiptDelete($id)
    {
        ...
        return redirect()->to('production/yarn-twisting/view/' . $dcId)->with('success', 'Receipt deleted and inventory reverted successfully.');
    }
}
