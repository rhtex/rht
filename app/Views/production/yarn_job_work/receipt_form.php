...
<?= $this->section('title') ?><?= isset($isEdit) ? 'Edit Receipt' : 'Receive Yarn Stock' ?><?= $this->endSection() ?>
...
<div class="row mb-2">
    ...
    <div class="col-sm-6 text-end">
        <a href="<?= site_url('production/yarn-job-work/view/'.$dc['id']) ?>" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Back to DC</a>
    </div>
</div>
<?= $this->endSection() ?>
...
<div class="card card-outline card-success">
    <?php 
        $formAction = isset($isEdit) 
            ? site_url('production/yarn-job-work/receipt-update/'.$receipt['id']) 
            : site_url('production/yarn-job-work/receipt-store/'.$dc['id']);
        $isWarpingSizing = in_array($dc['job_work_type'], ['Warping & Sizing', 'Warping', 'Sizing']);
    ?>
        ...
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control" value="<?= esc($dc['vendor_name']) ?> (<?= esc($dc['job_work_type']) ?>)" disabled>
                </div>
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <textarea name="remarks" class="form-control" rows="2" placeholder="Receipt notes..."><?= old('remarks', isset($isEdit) ? $receipt['remarks'] : '') ?></textarea>
                </div>
            </div>

            <!-- Warping & Sizing Beams Return Section -->
            <?php if ($isWarpingSizing): ?>
            ...
            <div class="row mt-4">
                ...
                <div class="col-md-12 table-responsive">
                    <?php if ($isWarpingSizing): ?>
                        <!-- WARPING & SIZING TABLE (Issued Qty, Recd Qty, Used Qty, Used Yarn Cost, Landed Cost) -->
                        <table class="table table-bordered align-middle" id="receiptItemsTable">
                            <thead>
                                <tr class="bg-light text-sm">
                                    <th>Yarn Description</th>
                                    <th width="10%">Issued Qty (Kg)</th>
                                    <th width="12%">Already Recd (Kg)</th>
                                    <th width="15%">Recd Qty (Loaded) (Kg) <span class="text-danger">*</span></th>
                                    <th width="15%">Used Qty (Kg) <span class="text-danger">*</span></th>
                                    <th width="12%">Used Yarn Cost (₹)</th>
                                    <th width="12%">Job Charges (₹/Kg) <span class="text-danger">*</span></th>
                                    <th width="15%">Received Yarn Cost (Landed) (₹)</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <?php 
                                    ...
                                    // Used Qty = Received Qty + Wastage
                                    $qtyUsedVal = $ri ? ($qtyReceivedVal + (float)$ri['quantity_wastage_kg']) : 0.00;
                                    $jobChargesVal = $ri ? (float)$ri['job_work_charges'] : 0.00;
                                ...
                                <tr class="item-row" data-item-id="<?= $item['id'] ?>" data-issued="<?= $issued ?>" data-pending="<?= $pending ?>" data-raw-cost="<?= $rawCost ?>">
                                    ...
                                    <td>
                                        <span class="fw-bold" id="lbl-used-yarn-cost-<?= $item['id'] ?>">₹0.00</span>
                                    </td>
                                    <td>
                                        <input type="number" step="0.01" name="items[<?= $item['id'] ?>][job_work_charges]" class="form-control form-control-sm input-job-charges" value="<?= number_format($jobChargesVal, 2, '.', '') ?>" min="0" required placeholder="₹/Kg">
                                    </td>
                                    <td>
                    ...
                    <?php else: ?>
                        <!-- DYEING / GENERIC TABLE -->
                        <table class="table table-bordered align-middle" id="receiptItemsTable">
                            <thead>
                                <tr class="bg-light text-sm">
                                    <th>Yarn Description</th>
                                    <th width="12%">Color</th>
                                    <?php if ($dc['job_work_type'] === 'Dyeing'): ?>
                                        <th width="12%">Dyeing Lot No</th>
                                    <?php endif; ?>
                            ...
                            <tbody>
                                ...
                                <?php 
                                    ...
                                    $qtyReceivedVal = $ri ? (float)$ri['quantity_received_kg'] : 0.00;
                                    $qtyWastageVal = $ri ? (float)$ri['quantity_wastage_kg'] : 0.00;
                                    $jobChargesVal = $ri ? (float)$ri['job_work_charges'] : 0.00;

                                    $defaultColor = $ri ? $ri['received_color'] : (($dc['job_work_type'] === 'Dyeing') ? ($item['required_color'] ?: 'Raw') : ($item['current_color'] ?: 'Raw'));
                                    ...
                                    if (isset($isEdit) && $ri) {
                                        $m = $db->table('production_yarn_stock_movements')
                                            ...
                                            ->where('brand_mill', $item['mill_name'])
                                            ->get()->getRowArray();
                                        $dyeingLotNumberVal = $m ? $m['lot_number'] : '';
                                    }
                                ...
                                <tr class="item-row" data-item-id="<?= $item['id'] ?>" data-issued="<?= $issued ?>" data-pending="<?= $pending ?>" data-raw-cost="<?= $rawCost ?>">
                                    <td>
                                        <strong><?= esc($item['mill_name']) ?> - <?= esc($item['yarn_count']) ?></strong><br>
                                        <small class="text-muted">
                                            Type: <?= esc($item['yarn_type']) ?> | Warp/Weft: <?= esc($item['warp_weft']) ?><br>
                                            Lot: <?= esc($item['lot_number'] ?: '-') ?> | CSP: <?= esc($item['csp'] ?: '-') ?>
                                        </small>
                                    </td>
                                    <td>
                                        <strong><?= esc($defaultColor) ?></strong>
                                        <input type="hidden" name="items[<?= $item['id'] ?>][received_color]" value="<?= esc($defaultColor) ?>">
                                    </td>
                                    <?php if ($dc['job_work_type'] === 'Dyeing'): ?>
                                        <td>
                                            <input type="text" name="items[<?= $item['id'] ?>][dyeing_lot_number]" class="form-control form-control-sm" value="<?= esc($dyeingLotNumberVal) ?>" placeholder="e.g. DYE-101">
                                        </td>
                                    <?php endif; ?>
                                    <td><?= number_format($issued, 2) ?> Kg</td>
                                    <td><?= number_format($receivedExcludingCurrent, 2) ?> Kg</td>
                                    ...
                                    <td>
                                        <input type="number" step="0.01" name="items[<?= $item['id'] ?>][quantity_wastage_kg]" class="form-control form-control-sm input-wastage" value="<?= number_format($qtyWastageVal, 2, '.', '') ?>" min="0" required>
                                    </td>
                                    <td>
                                        <strong class="text-primary"><span class="lbl-pending-kgs"><?= number_format(max(0, $pending - $qtyReceivedVal - $qtyWastageVal), 2, '.', '') ?></span> Kg</strong>
                                    </td>
                                    <td>
                                        <input type="number" step="0.01" name="items[<?= $item['id'] ?>][job_work_charges]" class="form-control form-control-sm input-job-charges" value="<?= number_format($jobChargesVal, 2, '.', '') ?>" min="0" required placeholder="₹/Kg">
                                    </td>
                                    <td>
                                        <div class="text-xs">
                                            <strong>Wastage:</strong> <span class="lbl-wastage-pct text-warning">0.00%</span><br>
                                            <strong>Landed:</strong> <span class="lbl-landed-cost text-success font-weight-bold">₹0.00</span>
                                        </div>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
...
<script>
    $(document).ready(function() {
        var isWarpingSizing = <?= $isWarpingSizing ? 'true' : 'false' ?>;
        ...
        // Auto-calculate wastage for dyeing/generic, or calculate used qty for warping/sizing
        $(document).on('input', '.input-recd, .input-used-qty, .input-wastage', function() {
            ...
            if (isWarpingSizing) {
                ...
                if (recd + wastage > pending) {
                    $row.find('.qty-error-msg').text('Total exceeds pending (' + pending.toFixed(2) + ' Kg)').show();
                    $row.addClass('table-danger');
                } else {
                    $row.find('.qty-error-msg').hide();
                    $row.removeClass('table-danger');
                ...
                var remainingPending = Math.max(0, pending - recd - wastage);
                $row.find('.lbl-pending-kgs').text(remainingPending.toFixed(2));
            }
        ...
        function validateSubmitButton() {
            ...
            $('.item-row').each(function() {
                ...
                if (isWarpingSizing) {
                    ...
                    var wastage = parseFloat($(this).find('.input-wastage').val()) || 0;
                    if (recd + wastage > pending) hasError = true;
                }
            });
        ...
        function calculateLandedCosts() {
            ...
            $('.item-row').each(function() {
                ...
                if (isWarpingSizing) {
                    ...
                    // Received Yarn Cost (Landed) = Used Yarn Cost + (Received Qty * Job Charges) + Share of Expenses
                    var landedCost = usedYarnCost + (recd * jobRate) + shareOfExpense;
                    ...
                    var wastage = parseFloat($row.find('.input-wastage').val()) || 0;
                    var wastagePct = (recd + wastage) > 0 ? ((wastage / (recd + wastage)) * 100) : 0;
                    $row.find('.lbl-wastage-pct').text(wastagePct.toFixed(2) + '%');

                    var rawCostConsumed = (recd + wastage) * rawCost;
                    var totalLandedCost = rawCostConsumed + (recd * jobRate) + shareOfExpense;
                    var landedCostPerKg = recd > 0 ? (totalLandedCost / recd) : 0;

                    $row.find('.lbl-landed-cost').text('₹' + landedCostPerKg.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
                }
            });

            // Calculate overall per meter for warping and sizing
            if (isWarpingSizing) {
                var totalMeters = 0;
